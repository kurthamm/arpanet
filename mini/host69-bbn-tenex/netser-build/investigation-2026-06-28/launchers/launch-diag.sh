...
# diag-buffers.do. Does NOT restart imp05 (only starts it if absent).
set -u
...
: > "$H/logs/dc16945.log"; : > "$CLOG"; : > "$H/logs/host69-diag.log"
...
nohup nice -n 5 "$BIN" "$DO" > "$H/logs/host69-diag.log" 2>&1 </dev/null &
echo "[L] ki pid $!"
