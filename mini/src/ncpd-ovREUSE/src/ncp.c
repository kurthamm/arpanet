...
// OSCAR - Forward declaration needed for process_reset() to call ncp_reset()
...
static void debug_print_listeners(void) {
    ...
    for (int i = 0; i < CONNECTIONS; i++) {
        if (listening[i].sock != 0) {
            fprintf(stderr, "  [Index %d] -> Socket %u\n", i, listening[i].sock);
            found = 1;
        }
    }
    if (!found) {
        fprintf(stderr, "  (listening array is empty)\n");
    }
    fprintf(stderr, "--------------------------------\n");
}
...
{
  fprintf(stderr, "DEBUG: destroy: Cleaning up connection at index %d.\n", i);
...
static void process_reset (uint8_t *packet, int length)
{
  ...
  // OSCAR - reconnect to restarted IMP
  // When the H316 IMP restarts, it loses all connection state.
  // We must clear our local state (connections, listeners, hosts) and
  // re-establish communication by:
  //   1. Clearing all connections and listening sockets via reset()
  //   2. Flapping the host ready signal (0 then 1) to re-handshake with IMP
  //   3. Sending NOPs to synchronize with the restarted IMP
  // This makes ncpd behave as if freshly started, allowing seamless reconnection.
  ncp_reset(1);
  // OSCAR - end of change
}
...
static void app_listen (void)
{
  ...
  fprintf(stderr, "DEBUG: app_listen: Added new listener for socket %u at index %d.\n", socket, i);
...
void ncp_init (void)
{
  ...
  // OSCAR - remove stale socket file to allow restart after crash
  // When ncpd crashes (SIGSEGV, SIGKILL, power loss, etc.), the Unix domain
  // socket file persists on the filesystem. Without this unlink(), attempting
  // to bind() on restart fails with "Address already in use" (EADDRINUSE).
  // Calling unlink() before bind() is the standard pattern for Unix domain
  // sockets - it removes any stale socket file from a previous run, allowing
  // seamless recovery. If the file doesn't exist, unlink() fails harmlessly.
  unlink (server.sun_path);
  // OSCAR - end of change
