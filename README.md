...
Host `49` / octal `061` is **HAMM-KA0** ("Hamm Computer Laboratory"), Kurt's physical PiDP-10 (a KA10 running ITS) at IMP #49 host 0. It is a deliberately *fictional* private ARPANET site placed at an IMP number that was vacant in the Dec 1973 host status (RFC 597) — so it does not impersonate a real node. (It previously sat at host `41` / octal `051`, which is IMP 41 — the real Norwegian NORSAR node — and was renumbered to 49 to be genuinely unique.) It reaches the mesh over Tailscale through the IMP62↔IMP49 trunk; verify with `NCP=ncp31 ./ncp-ping -c1 49` and `@L 49`.
...
<br><br>
You might want to skip all the web stuff. A quick way straight onto your Arpanet is to run './do.sh 1'. That gets a TIP-like session; type @L 6 [return] and it will connect to MIT-MULTICS, type @L 65 [return] and it will connect to UCLA-CCN, type @L 126 [return] and it will connect to the Hilton conference-site KA host, type @L 134 [return] and it will connect to MIT-AI, or type @L 198 [return] and it will connect to MIT-ML.
<br><br>
...
There is a fully formed but so far small Arpanet in ./mini. It consists of a network of IMP routers, simulated host systems, and browser terminal bridges connecting to each other. The active public host set currently includes UCLA-NMC host 1 as a SIMH Sigma 7 CP-V system, MIT-MULTICS host 6 as a DPS8M/MR12.8 Multics system, UCLA-CCN host 65 as an OS/360 MVT/TCAM/TSO system with real FORTRAN IV G under Hercules, MIT ITS hosts 70, 126, 134, and 198, Stanford/SU-AI host 11 running WAITS with restored PARRY support files, and the physical PiDP-10 (HAMM-KA0) at host 49. Dormant local ITS disk sets remain available for additional MIT hosts, but they are not exposed by default. Host 11 is reached through a Linux bridge rather than through its own recovered WAITS NCP. The UCLA host 1 path is a browser terminal route to a real Sigma emulator and CP-V media, not recovered UCLA-NMC SEX media. The UCLA-CCN host 65 path is a real OS/360 MVT/TSO/FORTRAN implementation for the historical host number, not recovered UCLA 360/91 storage and not recovered SPEAKEASY. Longer term, the hope is for deeper Multics NCP integration, PDP-11s, and additional historically grounded non-MIT hosts. It is early days yet. But the IMPs run reliably, connecting to make their network over simulated leased lines; and we're happy with progress :-)
<br><br>
